...
if __name__ == "__main__":
    ...
    # example train weights (units consistent with reactions you want)
    bp = BridgeProject([400/3, 400/3, 400/3], cross_section)

    bp.print_cross_section_properties()
    bp.draw_cross_section()

    V_example = 257.33  # N
    M_example = 69445.33  # N·mm
    # choose glue line (user input) - example: glue between bottom flange and web at y=20
    y_glue_example = 75
    b_glue_example = 5 * 2

    sigma_top, sigma_bottom, tau_centroid, tau_glue = bp.applied_stresses(V_example, M_example, y_glue_example, b_glue_example)
    # print(sigma_top, sigma_bottom, tau_centroid, tau_glue)

    fos = bp.factor_of_safety(V_example, M_example, y_glue_example, b_glue_example, 400, 76.27, 1.27)

    # bp.plot_buckling_sections()


    # buckling_panels = bp.buckling_stress_horizontal_plate()

    # for case, sigmas in buckling_panels.items():
    #     for sigma_cr in sigmas:
    #         print("Buckling case:", case, "sigma_cr:", sigma_cr)

    # print("Shear buckling sigma_cr:", bp.shear_buckling(400, 73.73, 1.27))